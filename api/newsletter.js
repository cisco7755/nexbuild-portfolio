...
export default async function handler(req, res) {
  ...
  try {
    await resend.emails.send({
      from: 'Nexbuild <onboarding@resend.dev>',
      ...
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px; background: #f8fafc; border-radius: 12px;">
          ...
          <p style="color: #334155; font-size: 15px; margin: 0 0 8px;">
            <strong>${email}</strong> just subscribed to Nexbuild Insights.
          </p>
    ...
    await resend.emails.send({
      from: 'Nexbuild <onboarding@resend.dev>',
      to: email,
      subject: "You're subscribed to Nexbuild Insights",
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px; background: #f8fafc; border-radius: 12px;">
          <div style="background: #4f46e5; border-radius: 8px; padding: 20px 24px; margin-bottom: 24px;">
            ...
            <p style="color: #c7d2fe; margin: 4px 0 0; font-size: 14px;">Nexbuild Insights</p>
          </div>
          ...
          <a href="https://nexbuilds.vercel.app/insights" style="display: inline-block; padding: 12px 24px; background: #4f46e5; color: white; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">
            Read our latest articles →
          </a>
          <p style="color: #94a3b8; font-size: 12px; margin-top: 24px;">Nexbuild · Lagos, Nigeria · hello@nexbuild.io</p>
        </div>
      `,
    })
